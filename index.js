...
const antinsta = JSON.parse(fs.readFileSync('./src/antiinsta.json'))
...
case 'dgc':
	...
	if (budy.startsWith(`Pongan cuties`)) { 
        ...
        if (budy.includes(`${prefix}reglas`)) {
        reply(`I: Respeto y bienestar
⤿ Tener respeto mutuo es importante para mantener una comunidad no tóxica, por lo que se prohíbe tratar con cierta agresividad a alguien. Si no se define el provocante de un bardo, serán sancionados los participantes de la misma.

II: NSFW y gore
⤿ La pornografía, imágenes sensibles, sangrientas suelen ser traumantes para cualquiera e incluso un delito. Se prohíbe todo tipo de archivo relacionado con esto.

III: Lenguaje inclusivo
⤿ A partir de aquí se pueden generar conflictos, pero no permitiremos el polémico lenguaje inclusivo ('todxs/todes', 'chiquxs/chiques'), resulta molesto e ilegible el mensaje. Si no estas de acuerdo con esto, lo sentimos.

IV: Promociones
⤿ Entendemos que podemos servir para ayuda, pero eso no significa aceptar promover el spam. Se prohíbe la promoción de redes sociales o marcas, incluyendo audiovisuales y enlaces con una promoción por encima (a la vista). Esto también incluye el spam al MD.

V: Enlaces/links
⤿ Para evitar incumplir la regla IV, se prohíbe el uso de links/enlaces, excepto los enlaces de archivo compartido en Google Drive.

VI: Acoso
⤿ Persisitir intensamente a una persona puede generar miedo y pánico. Deja en paz a los usuarios.

VIII: Flood y tags masivos
⤿ El flood (inundación del grupo) es uno de los métodos más fuertes y desesperantes que se utiliza para invadir un grupo, server, etc., saturándolo de mensajes basura e impidiendo comunicarse con normalidad. El exceso de repetidos mensajes/caracteres consecutivos provoca flood, lo mismo incluye a las menciones con "@". Se prohíbe este acto.

IX: Molestar al MD
 ⤿ Incumplir las reglas al MD suele ser una buena estrategia para evitar ser sancionado, pero no siempre. No queremos dar muchos detalles, en resumen, se permite charlar con los miembros del R.A por MD mientras no se realicen actos molestos para el prójimo (insultar, spamear, etc.). Para esta regla, necesitamos tu ayuda de avisarnos (con pruebas) sobre el hecho.

X: Conducta moral y ética
⤿ Está más que claro que todos conocemos lo que es un comportamiento correcto e incorrecto a lo largo de los años, por ejemplo, ¿ustedes dónde clasificarían los robos o las estafas? ¿es un delito o legal?. Las respuestas son claras gracias a la educación moral y ética que hemos adquirido.`)}
